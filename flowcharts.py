...
def rtp_application(out_dir, owner=None):
    ...
    _proc(g, "d1", "Component 1 — Organisation\nRegistration (OR) via TPGateway")
    _proc(g, "d2", "Component 2 — concurrent\nCourse Application (CA)")
    _proc(g, "e", "Pay both fees: OR S$545 +\nCA per course (S$109 / S$654, GST incl.)")
    ...
    g.edge("start", "a"); g.edge("a", "b"); g.edge("b", "c")
    g.edge("c", "d1"); g.edge("c", "d2")
    g.edge("d1", "e"); g.edge("d2", "e"); g.edge("e", "f"); g.edge("f", "gg")
